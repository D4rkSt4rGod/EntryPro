console.log('[Entry Pro] inject.js running');

(function() {

    if (typeof window.entryProRemoved === 'undefined') {
        window.entryProRemoved = false;
    }

    // if already removed, skip injection
    if (window.entryProRemoved) {
        console.log('[Entry Pro] Script is marked as removed. Skipping injection.');
        return;
    }

    if (window.__entryProLoaded) return;
    window.__entryProLoaded = true;

    console.log('[Entry Pro] Starting Entry Pro with NPI integration...');

    // waiting for Entry to be ready
    function waitForEntry(callback) {
        if (window.Entry && window.Entry.block && window.Entry.playground) {
            console.log('[Entry Pro] Entry is ready');
            callback();
        } else {
            console.log('[Entry Pro] Waiting for Entry...');
            setTimeout(() => waitForEntry(callback), 500);
        }
    }
    ...
    function init() {
        console.log('[Entry Pro] Initializing...');
        ...
        addBlock('window2', '%1 내용의 %2 창 띄워주고 기다리기%3', {
            ...
            def: [
                ...
                { type: 'text', params: ['NPI'] },

                { type: 'Dropdown', params: ['alert'] },
            ...
            switch (type) {
                ...
                case 'confirm':
                    ...
                    return script.callReturn(confirm(message));

                case 'prompt':

                    return script.callReturn(prompt(message));
        ...
        Entry.staticBlocks.push({
            ...
            category: 'API', blocks: [
                ...
                'site','move','Search','illegal','npi off','off'
        ...
        console.log('[Entry Pro] Initialization complete');
    }

    // If Entry Loaded, execute callback immediately; else wait until loaded
    waitForEntry(init);
    
    // Check After Page Loaded
    window.addEventListener('load', () => {
        console.log('[Entry Pro] Page loaded');
        setTimeout(() => waitForEntry(init), 1000);
    });
    ...
    window.EntryPro = {
        version: '1.0.0',
        refresh: function() {
            console.log('[Entry Pro] Manual refresh requested');
        },
        getStatus: function() {
            return {
                entryLoaded: !!window.Entry,
                npiLoaded: typeof window.addBlock === 'function',
                categoryAdded: !!document.getElementById('entryCategoryentrypro')
            };
        }
    };
